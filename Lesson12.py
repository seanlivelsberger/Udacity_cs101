...
def total_enrollment(univ_list):
    enrollment = 0
    tuition_fees = 0
    for x in univ_list:
        enrollment = enrollment + x[1]
        tuition_fees = tuition_fees + x[1]*x[2]
...
def crawl_web(seed,max_depth):
    ...
    while max_depth >= 0:
        ...
        while i > 0:
            page = tocrawl.pop()
            if page not in crawled:
                union(newcrawl,get_all_links(get_page(page)))
                crawled.append(page)
            i = i - 1
        union(tocrawl, newcrawl)
